...
const { Util: { escapeMarkdown }, TextChannel, GuildChannel } = require('discord.js')
...
class TextChannelArgumentType extends ArgumentType {
	 ...
	 * @param {CommandoMessage} msg Message that triggered the command
	 * @return {TextChannel} Usable value
	 */
	parse(val, msg) {
		const matches = val.match(/^(?:<#)?([0-9]+)>?$/)
...
function channelFilterExact(search) {
	return /** @param {GuildChannel} chan */ chan =>
		chan.type === 'GUILD_TEXT' && chan.name.toLowerCase() === search
}

/** @param {string} search */
function channelFilterInexact(search) {
	return /** @param {GuildChannel} chan */ chan =>
		chan.type === 'GUILD_TEXT' && chan.name.toLowerCase().includes(search)
}
