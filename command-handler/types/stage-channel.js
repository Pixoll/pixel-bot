...
const { Util: { escapeMarkdown }, GuildChannel, StageChannel } = require('discord.js')
...
class StageChannelArgumentType extends ArgumentType {
	 ...
	 * @param {CommandoMessage} msg Message that triggered the command
	 * @return {StageChannel} Usable value
	 */
	parse(val, msg) {
		const matches = val.match(/^(?:<#)?([0-9]+)>?$/)
...
function channelFilterExact(search) {
	return /** @param {GuildChannel} chan */ chan =>
		chan.type === 'GUILD_STAGE_VOICE' && chan.name.toLowerCase() === search
}
